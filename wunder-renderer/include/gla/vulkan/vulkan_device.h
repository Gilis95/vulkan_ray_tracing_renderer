...
class vulkan_physical_device
{
 ...
 public:
  explicit vulkan_physical_device(VkInstance m_vk_instance);
  ~vulkan_physical_device();

  [[nodiscard]] bool is_extension_supported(const std::string& extensionName) const;
  uint32_t get_memory_type_index(uint32_t typeBits, VkMemoryPropertyFlags properties) const;

  [[nodiscard]] VkPhysicalDevice get_vulkan_physical_device() const { return m_physical_device; }
  [[nodiscard]] const queue_family_indices& get_queue_family_indices() const { return m_queue_family_indices; }

  [[nodiscard]] const VkPhysicalDeviceProperties& get_properties() const { return m_device_properties; }
  [[nodiscard]] const VkPhysicalDeviceLimits& get_limits() const { return m_device_properties.limits; }
  [[nodiscard]] const VkPhysicalDeviceMemoryProperties& get_memory_properties() const { return m_physical_device_memory_properties; }
