...
namespace wunder {
    ...
    VkResult vulkan_renderer::select_gpu() {
        ...
        auto result = vkEnumeratePhysicalDevices(m_vk_instance, &physical_device_count, nullptr);
        ...
        // device handles.
        std::vector<VkPhysicalDevice> physical_devices(physical_device_count); // list of available gpu`s
        vkEnumeratePhysicalDevices(m_vk_instance, &physical_device_count, &physical_devices[0]);

        for (auto physical_device: physical_devices) {
            VkPhysicalDeviceProperties properties;
            vkGetPhysicalDeviceProperties(physical_device, &properties);

            ContinueUnless(properties.deviceType == VkPhysicalDeviceType::VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU);
    ...
    VkResult vulkan_renderer::create_vulkan_logical_device() {
        ...
        create_info.queueCreateInfoCount = sizeof(queues_info) / sizeof(queues_info[0]);
...
//        return vkCreateDescriptorPool(m_device, &pool_info, nullptr, &m_descriptor_pool);
    }
}
